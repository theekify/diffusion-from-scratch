...
def main():
    ...
    fig, axes = plt.subplots(samples_per_epoch, len(epochs_to_show), figsize=(15, 12))
    ...
    for col, epoch in enumerate(epochs_to_show):
        ...
        images = sample(model, schedule, num_samples=samples_per_epoch, device=device)

        for row in range(samples_per_epoch):
    ...
    plt.tight_layout()
    plt.savefig("../outputs/samples/training_progression_multisample.png", dpi=150)
    plt.show()
